...
async def test_graph_classifies_and_drafts_response_for_sample_ticket():
    client = FakeOpenRouterClient(
        [
            ...
            "I’m sorry your package has not arrived. Could you share your order number "
            "so we can look into its status?",
        ]
    )
    graph = build_graph(client=client, primary_model="test-model")

    result = await graph.ainvoke(
        {"ticket_text": "My package has not arrived. Can you check its status?"}
    )

    assert result["ticket_text"] == "My package has not arrived. Can you check its status?"
...
async def test_invalid_classification_fails_without_retry(classification: str):
    ...
    with pytest.raises(ValueError):
        await graph.ainvoke({"ticket_text": "Where is my package?"})
